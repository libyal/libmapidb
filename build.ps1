...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libmapidb.sln 2>&1 | %{ '$_' }"
	}
}
$VSSolutionFile = "${VSSolutionPath}\libmapidb.sln"
