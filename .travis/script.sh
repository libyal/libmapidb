...
then
	docker run -t -v "${PWD}:/libmapidb" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libmapidb && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libmapidb;
